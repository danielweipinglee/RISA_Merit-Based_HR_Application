...
CREATE TABLE IF NOT EXISTS `RISA_HR`.`Student` (
  ...
  CONSTRAINT `fk_Student_StudentPassword1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Student_RISAPosition1`
    FOREIGN KEY (`RISAPosition_ID`)
    REFERENCES `RISA_HR`.`RISAPosition` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
